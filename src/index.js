    ...
    const clickListener = () => {
        document.addEventListener('click', function(e){
            if (e.target.matches('.btn-danger')){
                ...
                if (e.target.innerText === "Sort by Author Name") {     
                    fetch(baseURL)
                    .then(res => res.json())
                    .then(data => {
                        const compare = (a, b)  => {
                            const authA = a.author.toUpperCase();
                            const authB = b.author.toUpperCase();                      
                            let comparison = 0;
                            if (authA > authB) {
                            comparison = 1;
                            } else if (authA < authB) {
                ...
                } else if (e.target.innerText === "Unsort") {
                    quoteList.innerHTML = ""
                    getQuotes()
                    e.target.innerText = "Sort by Author Name"
                }
            }
        })
    }
